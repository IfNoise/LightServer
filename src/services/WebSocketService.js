...
class WebSocketService {
  ...
  init(server, port = null) {
    ...
    if (port) {
      server.listen(port, () => {
        logger.info('WebSocket server listening', { port, path: '/ws/channels' });
      });
    }
    ...
    this.wss.on('connection', (ws, req) => {
      ...
      ws.on('message', (message) => {
        try {
          ...
          logger.error('Failed to parse WebSocket message', { error: e.message });
        }
      });
  ...
  broadcast(data) {
    ...
    this.clients.forEach((client) => {
      if (client.readyState === 1) { // WebSocket.OPEN
        try {
          ...
          logger.error('Failed to send message to WebSocket client', { error: e.message });
          errorCount++;
    ...
    if (sentCount > 0) {
      logger.debug('Broadcast message sent', { sentCount, errorCount, type: data.type });
    }
  }
